  ...
  return <main className="contact-page">
    ...
    <div className="contact-flow">
      ...
      <section className="section contact-visit-section"><div className="shell contact-visit-layout">
        <Reveal className="contact-visit-copy"><SectionLabel>{contact.visit.eyebrow}</SectionLabel><h2>{contact.visit.title}</h2><address>Teaxini<br />1st Floor, 1D-1, Jalan SS 9a/17,<br />43700 Petaling Jaya, Selangor, Malaysia</address><div className="hero-actions"><a className="button button-primary" href={mapLink} target="_blank" rel="noopener noreferrer">{contact.visit.googleMaps} <ArrowRight /></a><a className="button button-ghost" href={wazeLink} target="_blank" rel="noopener noreferrer">{contact.visit.waze} <ArrowRight /></a></div><nav className="contact-socials" aria-label={social.followUs}><SectionLabel>{social.followUs}</SectionLabel><a href={socialLinks.facebook} target="_blank" rel="noopener noreferrer" aria-label={social.visitFacebook}><span className="contact-social-icon"><FacebookIcon className="facebook-icon" /></span><span className="contact-social-account"><small>{social.facebook}</small><b>{social.facebookName}</b></span><ArrowUpRight /></a><a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" aria-label={social.visitInstagram}><span className="contact-social-icon"><InstagramIcon className="instagram-icon" /></span><span className="contact-social-account"><small>{social.instagram}</small><b>{social.instagramName}</b></span><ArrowUpRight /></a></nav></Reveal>
